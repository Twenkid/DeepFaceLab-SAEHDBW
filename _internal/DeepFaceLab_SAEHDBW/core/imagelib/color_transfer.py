...
print(f"color_transfer.use_bw_input={use_bw_input}")
...
#27-5-2022: Try mkl, idt; again linear lct: when releasing the RAM disk? - it requests 5 GB RAM allocation if the code is converted correctly?  Resolved: an error, I had to reshape it to the target img, e.g. (192,192,1), instead the current shape was returned (1, 36864); fixed, missed one reshape which was conditional

def color_transfer_sot(src,trg, steps=10, batch_size=5, reg_sigmaXY=16.0, reg_sigmaV=5.0):
    if debug_color_transfer: print(f"color_transfer_sot, {src.shape}, {trg.shape}") #26-5-2022
    if len(src.shape)==2: src = src[...,np.newaxis]
    if len(trg.shape)==2: trg = trg[:,:,np.newaxis] #[..., np.newaxis] it's the same
    if debug_color_transfer: print(f"color_transfer_sot, {src.shape}, {trg.shape}") 
    """
...
def reinhard_color_transfer(target : np.ndarray, source : np.ndarray, target_mask : np.ndarray = None, source_mask : np.ndarray = None, mask_cutoff=0.5) -> np.ndarray:
    ...
    """
    print("core\imagelib\colort_transfer.py\reinhard_color_transfer rct")
...
def linear_color_transfer(target_img, source_img, mode='pca', eps=1e-5):
    ...
    if debug_color_transfer: print(f"color_transfer.py: linear_color_transfer,target_img.shape={target_img.shape}, source_img.shape={source_img.shape}")    
    ...
    if use_bw_input: #18-5-2022
      ...
      if s1 == 1: target_img = np.squeeze(target_img); target_img = target_img[...,np.newaxis] # for mergings
    ...
    if not use_bw_input: #24-5-2022 --> that must be the colors, BGR to RGB
      ...
      if debug_color_transfer: print(f"t.shape={t.shape}")
      t = t.reshape( t.shape[-1],-1) #?24-5-2022  --> no transpose, but reshape? or don't?
      if debug_color_transfer: print(f"t.reshape(t.shape[-1],-1)={t.shape}")
    ...
    if not use_bw_input:
      ...
      if debug_color_transfer: print(f"s.shape={s.shape}")
    ...
    if use_bw_input and debug_color_transfer: 
...
def bw_hist_match(src_im, tar_im, hist_match_threshold=255): #22-5-20222
    h,w,c = src_im.shape
    matched = channel_hist_match(src_im[0], tar_im[0], hist_match_threshold, None)
