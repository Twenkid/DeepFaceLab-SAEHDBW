...
def init_weights(weights):
    print("lears\ops\__init__.py: def init_weights")
    ops = []
...
def dssim(img1,img2, max_val, filter_size=11, filter_sigma=1.5, k1=0.01, k2=0.03):
    print("IN dssim")
    print(f'img1={img1.shape}, img2 = {img2.shape}, max_val={max_val}')    
...
def depth_to_space(x, size):
    print(f"depth_to_space: core\leras\ops\__init__.py x={x}, size={size}, x.shape={x.shape}, x.shape[0]={x.shape[0]}, 1={x.shape[1]}, 2={x.shape[2]},3={x.shape[3]}")
    """
    if x.shape[1]%4 !=0: #6-5-2022
      print("shape not divisible by 4!")
      aligned = int ( (x.shape[1]//4) * 4)
      print(f"aligned = {aligned}")
      y = tf.slice(x, begin=[0,0,0,0], size=[int(x.shape[0]),aligned, int(x.shape[2]), int(x.shape[3])])
      x = y
      print(x)
      ?, 769,24, 24
      y = tf.slice(x, begin=[0,0,0,0], size=[int(x.shape[0]),aligned, int(x.shape[2]), int(x.shape[3])])
    ...
    if nn.data_format == "NHWC":
        # match NCHW version in order to switch data_format without problems
        print(f"if nn.data_format == NHWC: core\leras\ops\__init__.py\n x={x}\nsize={size}")
        b,h,w,c = x.shape.as_list() #batch, height, widtch, channels
        print(f" b,h,w,c = {b}, {h}, {w}, {c}")
        oh, ow = h * size, w * size
        print(f"oh, ow = h * size, w * size ... \n oh={oh}, ow={ow}")
        oc = c // (size * size)
        print(f"AFTER: oc = c // (size * size) \n oc={oc}, c={ow}, {size} * {size}")
        print(f"BEFORE: x = tf.reshape(x, (-1, h={h}, w={w}, size={size}, oc={oc}, )\n {x}, (-1, {h}, {w}, {size}, {oc})")
        x = tf.reshape(x, (-1, h, w, size, size, oc, ) )
        print(f"BEFORE tf.transpose(...)")
        x = tf.transpose(x, (0, 1, 3, 2, 4, 5))
        print(f"BEFORE x = tf.reshape(x, (-1, oh, ow, oc, )) == {x} = ... {x}, (-1, {oh}, {ow}, {oc}, ))")
        x = tf.reshape(x, (-1, oh, ow, oc, ))
        print(f"depth_to_space: return x = {x}")
        return x
    else:
        print(f"else: if nn.data_format != NHWC: core\leras\ops\__init__.py\n x={x}\nsize={size}\nformat=NCHW")
        cfg = nn.getCurrentDeviceConfig()
        if not cfg.cpu_only:
            print(f"if not cfg.cpu_only:... return tf.depth_to_space(x, size, data_format=nn.data_format), x={x}, size={size}, data_format={nn.data_format}")
            return tf.depth_to_space(x, size, data_format=nn.data_format)
        b,c,h,w = x.shape.as_list()
        print(f" b,c,h,w = {b}, {c}, {h}, {w}")
        oh, ow = h * size, w * size
        print(f"oh, ow = h*size, w*size = {oh}, {ow} = {h}*{size}, {w}*{size}")
        oc = c // (size * size)
        print(f"oc = c // (size * size) = {oc} = {c} // {size}*{size}")
        print(f"BEFORE: tf.reshape(x, (-1, size, size, oc, h, w, ) ) = {x}, (-1, {size}, {size}, {oc}, {h}, {w}, ")
        x = tf.reshape(x, (-1, size, size, oc, h, w, ) )
        print(f"BEFORE: tf.transpose(x, (0, 3, 4, 1, 5, 2))")        
        x = tf.transpose(x, (0, 3, 4, 1, 5, 2))
        print(f"BEFORE: x = tf.reshape(x, (-1, oc, oh, ow))\n{x} = tf.reshape({x}, (-1, {oc}, {oh}, {ow}")  
        x = tf.reshape(x, (-1, oc, oh, ow))
        print(f"return x={x}")
        return x
