...
print("nn.py: use_color_input_and_grayscale_model=? ",use_color_input_and_grayscale_model)

conv2d_ch_axis_forced = 3 #the same
# if use_color_input_and_grayscale_model: conv2d_ch_axis_forced = 1 #NO!? The same axis? #23-4-2022
...
class nn():
    ...
    def set_data_format(data_format):
        ...
        if data_format == "NHWC":
            nn.conv2d_ch_axis = 3 
            #nn.conv2d_ch_axis = conv2d_ch_axis_forced #conv2d_ch_axis #3             
            nn.conv2d_spatial_axes = [1,2] #aha, 0: N, 1: Height, 2: Width
        elif data_format == "NCHW":
            nn.conv2d_ch_axis = 1
            nn.conv2d_spatial_axes = [2,3] # 0,1 ... 2:H, 3:W 
        print(f"def set_data_format(data_format): data_format={data_format}, nn.conv2d_ch_axis={nn.conv2d_ch_axis},nn.conv2d_spatial_axes = {nn.conv2d_spatial_axes}") 
    ...
    def get4Dshape ( w, h, c ):
        ...
        if nn.data_format == "NHWC":
            #24-4-2022
            print(f"nn.data_format == NHWC None, {h}, {w}, {c}")
            return (None,h,w,c)
        else:
            print(f"nn.data_format == NCHW None, {c}, {h}, {w}")
            return (None,c,h,w)
