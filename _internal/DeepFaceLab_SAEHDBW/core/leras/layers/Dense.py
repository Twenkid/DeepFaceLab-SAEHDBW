...
import os
debug_dense_py = "debug_dense_py" in os.environ #6-5-2022
...
class Dense(nn.LayerBase):
    def __init__(self, in_ch, out_ch, use_bias=True, use_wscale=False, maxout_ch=0, kernel_initializer=None, bias_initializer=None, trainable=True, dtype=None, **kwargs ):
        ...
        """
        if debug_dense_py: print(f"Dense(nn.LayerBase) __init__: in_ch, out_ch, use_bias, use_wscale, maxout_ch,... = {in_ch}, {out_ch}, {use_bias}, {use_wscale}, {maxout_ch}, dtype={dtype} ...")
        self.in_ch = in_ch
        self.out_ch = out_ch
    ...
    def build_weights(self):
        ...
        if debug_dense_py: print(f"weight_shape={weight_shape}") #6-5-2022
    ...
    def forward(self, x):
        if debug_dense_py: print(f"Dense.forward(... self.weight={self.weight}")
        weight = self.weight
        if self.use_wscale:
