            ...
            class Decoder(nn.ModelBase):
                ...
                def forward(self, z):
                    ...
                    if 'd' in opts:
                        ...
                        ##### THAT CODE CALLS!!!! out_conv(x) etc... 
                        ##### It produces side effects etc. probably!!! OK, #24-4-2022 (st.), 25-4-2022, 6:25 - fixed by copying the original
                        #forced_nn_conv2d_ch_axis = nn.conv2d_ch_axis #
                        """
                        print("DeepFakeArchi.py")
                        print(f"Decoder.forward:x=tf.nn.sigmoid(nn.depth_to_space(tf.concat(((self.out_conv(x), self.out_conv1(x), self.out_conv2(x), self.out_conv3(x), nn.conv2d_ch_axis), 2))")
                        print(f"x={x}")
