...
debug_interact = False #input_bool, ... #20-4-2022; 30-4-2022
...
class InteractBase(object):
    ...
    def input_bool(self, s, default_value, help_message=None): #Added 1/0, Twenkid #20.4.2022
        s = f"[{yn_str[default_value]}] {s} (y/n or 1/0)"
        ...
        while True:
            try:
                ...
                #return bool ( {"y":True,"n":False, "1":True, "0":False}.get('0', 'n' )) #TEST             
                b = bool ( {"y":True,"n":False, "1":True, "0":False}.get(inp.lower(), default_value ))                
                if debug_interact: print(f"READ: {b}")
                return bool ( {"y":True,"n":False, "1":True, "0":False}.get(inp.lower(), default_value ))
            except:
                if debug_interact:
                  print ( "y" if default_value else "n" )
                  print ( "1/0 are also accepted for true/false in this fork (Twenkid)" )
