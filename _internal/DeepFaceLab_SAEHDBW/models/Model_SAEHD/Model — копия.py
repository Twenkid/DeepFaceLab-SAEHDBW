...
""").lower()
    ...
    def on_initialize(self):
        ...
        with tf.device ('/CPU:0'):
            ...
            self.warped_src2 = tf.placeholder (nn.floatx, bgr_shape, name='warped_src2')
            self.warped_dst2 = tf.placeholder (nn.floatx, bgr_shape, name='warped_dst2')
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    with tf.device(f'/CPU:0'):
                        ...
                        gpu_warped_src2     = self.warped_src2 [batch_slice,:,:,:]
                        gpu_warped_dst2     = self.warped_dst2 [batch_slice,:,:,:]
                    ...
                    if 'df' in archi_type:
                        ...
                        gpu_src2_code = self.encoder (gpu_warped_src2)
                        gpu_src2_inter_AB_code = self.inter_AB (gpu_src2_code)
                        gpu_src2_code = tf.concat([gpu_src2_inter_AB_code,gpu_src2_inter_AB_code], nn.conv2d_ch_axis  )
                        
                        gpu_dst2_code = self.encoder (gpu_warped_dst2)
                        gpu_dst2_inter_B_code = self.inter_B (gpu_dst2_code)
                        gpu_dst2_inter_AB_code = self.inter_AB (gpu_dst2_code)
                        gpu_dst2_code = tf.concat([gpu_dst2_inter_B_code,gpu_dst2_inter_AB_code], nn.conv2d_ch_axis )
                        
                        gpu_pred_src_src2, gpu_pred_src_srcm2 = self.decoder(gpu_src2_code)
                        gpu_pred_dst_dst2, gpu_pred_dst_dstm2 = self.decoder(gpu_dst_code)
                    ...
                    # Invariance loss
                    #gpu_src_loss += tf.reduce_mean ( tf.square( gpu_pred_src_src - gpu_pred_src_src2 ),axis=[1,2,3] )
                    #gpu_dst_loss += tf.reduce_mean ( tf.square( gpu_pred_dst_dst - gpu_pred_dst_dst2 ),axis=[1,2,3] )
        ...
        if self.is_training:
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, random_ct_samples_path=random_ct_samples_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        sample_process_options=SampleProcessor.Options(scale_range=[-0.1, 0.1], random_flip=random_src_flip),
                        output_sample_types = [ {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'warp_rnd_seed_shift': 0, 'channel_type' : SampleProcessor.ChannelType.BGR, 'ct_mode': ct_mode,                                         'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
                                                {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'warp_rnd_seed_shift': 1, 'channel_type' : SampleProcessor.ChannelType.BGR, 'ct_mode': ct_mode,                                         'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
                    ...
                    SampleGeneratorFace(training_data_dst_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        sample_process_options=SampleProcessor.Options(scale_range=[-0.1, 0.1], random_flip=random_dst_flip),
                        output_sample_types = [ {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'warp_rnd_seed_shift': 0, 'channel_type' : SampleProcessor.ChannelType.BGR,                                                             'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
                                                {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True, 'warp_rnd_seed_shift': 1, 'channel_type' : SampleProcessor.ChannelType.BGR,                                                             'face_type':self.face_type, 'data_format':nn.data_format, 'resolution': resolution},
