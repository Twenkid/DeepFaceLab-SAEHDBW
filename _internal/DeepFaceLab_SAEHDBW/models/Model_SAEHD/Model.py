...
bDebugOnTrainOneIter = False #25-4-2022
...
""").lower()
    ...
    def on_initialize(self):
        ...
        print(f"SAEHD COLOR: device_config={device_config}, devices={devices}")
        print(f"SAEHD COLOR: len(devices)={len(devices)}, self.is_debug={self.is_debug()}\nself.model_data_format = {self.model_data_format}")
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                #"""
                self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')
                print(f"SAEHD COLOR liae: self.encoder {self.encoder}, in_ch = {input_ch}")
                encoder_out_ch = self.encoder.get_out_ch()*self.encoder.get_out_res(resolution)**2
                ...
                self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')   #1, 64  -- #27-4-2022,        4:13  calculating the numbers
            ...
            #5-5-2022 - display more info in the preview window
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi_type:
                        ...
                        print(f'SAEHD liae gpu_src_code={gpu_src_code}, gpu_warped_src= {gpu_warped_src}') 
                        #AFTER self.encoder')
                        ...
                        print(f'SAEHD: gpu_pred_src_src, gpu_pred_src_srcm = {gpu_pred_src_src}, {gpu_pred_src_srcm}') 
        ...
        for model, filename in io.progress_bar_generator(self.model_filename_list, "Initializing models"):
            ...
            if not do_init:
                print("Model.py: if not do_init, filename, self.get_strpath_...")
                print(filename)
                print(self.get_strpath_storage_for_file(filename))
                
                do_init = not model.load_weights( self.get_strpath_storage_for_file(filename) )
                print("AFTER: do_init = not model.load_weights(...")
            if do_init:
                model.init_weights()
    ...
    def onTrainOneIter(self):
        ...
        if bDebugOnTrainOneIter:
          print("SAEHD-COLOR:{warped_src.shape}, {target_src.shape}, {target_srcm.shape}, {target_srcm_em.shape}, {warped_dst.shape}, {target_dst.shape}, {target_dstm.shape}, {target_dstm_em.shape}")
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        print(f"SAEHD *SUM MASKS: np.sum(DDM) = {np.sum(DDM)}, np.sum(SDM) = {np.sum(SDM)}")
