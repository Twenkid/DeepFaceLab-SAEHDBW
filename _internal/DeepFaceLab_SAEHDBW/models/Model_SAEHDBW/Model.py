...
#from core.leras.archis import DeepFakeArchiBWTEST #26-4-2022 no need to import here`
...
export_channels = 1 if use_color_input_and_grayscale_model == "color_input_and_grayscale_model" in os.environ else 3 #25-4-2022
bDebugModelSAEHDBW_onGetPreview = False #27-4-2022
bDebugMode_on_initialize = False 
#bDebug_onTrainOneIter = False
bDebug_onTrainOneIter = "debug_on_train_one_iter" in os.environ #renamed, #2-5-2022
print(f"SAEHDBW.Model.py: bDebug_onTrainOneIter={bDebug_onTrainOneIter}")
use_bw_input = "use_bw_input" in os.environ
print(f"SAEHDBW.Model.py: use_bw_input={use_bw_input}")

if "preview_period" in os.environ: preview_period = int(os.environ["preview_period"]) #3-5-2022
if "preview_period_colab" in os.environ: preview_period_colab = int(os.environ["preview_period_colab"])

#debug_model_saehdbw = "debug_model_saehdbw" in os.environ  #8-5-2022

debug_AE_view = "debug_AE_view" in os.environ

import cv2
from core.cv2ex import *

#class SAEHDModel(ModelBase):
class SAEHDModelBW(ModelBase): #25-4-2022 so far used the same model like for the color SAEHDModel ...
    ...
    def on_initialize_options(self):
        ...
        yn_str = {True:'1',False:'0'} #I had Cyrillic in console and had to switch it on each run. That can be fixed also by setting Latin as default layout in Windows, but 1/0 are also more accessible keys
        min_res = 64
        max_res = 640
...
""").lower()
        ...
        if self.is_first_run():
            self.options['ae_dims'] = np.clip ( io.input_int("AutoEncoder dimensions", default_ae_dims, add_info="32-1024", help_message="All face information will be packed to AE dims. If the amount of AE dims is not enough, then for example closed eyes will not be recognized. More dims are better, but require more VRAM. You can fine-tune model size to fit your GPU." ), 32, 1024 )
    ...
    def on_initialize(self):
        #if bDebugMode_on_initialize: print(f"SAEHDBW\Model.py on_initialize")
        device_config = nn.getCurrentDeviceConfig()
        ...
        if "color_input_and_grayscale_model" in os.environ or use_bw_input:
        ...
        #FORCED
        #input_ch = 1 #1 !!! On the first run the  read?
        #use_color_input_and_grayscale_model = True
        
        #bgr_shape = self.bgr_shape = nn.get4Dshape(resolution,resolution,input_ch) #26-4-2022 TRY ---> 3 channels and copy BW
        
        #input_ch = 3 #That's not color channels???? #27-4-2022, 4:59 --> input is not fed as separate RGB channels? these are predictions etc.? Src, Dst, SD? ... I don't know NO -- it is color channels. bgr_shape!
        ...
        #model_archi = nn.DeepFakeArchiBWTEST(resolution, use_fp16=use_fp16, opts=archi_opts) #26-4-2022
        model_archi = nn.DeepFakeArchiBWTEST_NOPRINT(resolution, use_fp16=use_fp16, opts=archi_opts) #26-4-2022               
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                print(f'encoder_out_ch={encoder_out_ch}, inter_out_ch = {inter_out_ch}')
                ...
                print("elif liae in archi type...")
                self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims, name='encoder')   #1, 64  -- or color : 3 #27-4-2022,        4:13  calculating the numbers
                print(f"in_ch = {input_ch}, e_ch={e_dims}")
                print(f"AFTER: self.encoder = model_archi.Encoder(in_ch=input_ch, e_ch=e_dims,...)... {input_ch} {e_dims}")
                encoder_out_ch = self.encoder.get_out_ch()*self.encoder.get_out_res(resolution)**2  #96*96 = 9216
                print(f"encoder_out_ch={self.encoder.get_out_ch()}*{self.encoder.get_out_res(resolution)**2}={encoder_out_ch}")
                print(f"encoder_out_ch={encoder_out_ch}")
                self.inter_AB = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=ae_dims*2, name='inter_AB') #in_ch = 9216, ae_ch = ae_dims = 64, ae_out_ch = 64*2  --> 9216, 64, 128 ?
                print(f"self.inter_AB, in_ch ={encoder_out_ch}, ae_ch = {ae_dims}, ae_out_ch = {ae_dims*2}")                
                self.inter_B  = model_archi.Inter(in_ch=encoder_out_ch, ae_ch=ae_dims, ae_out_ch=ae_dims*2, name='inter_B')
                #in_ch = 9216, ae_ch = ae_dims = 64, ae_out_ch = 64*2  --> 9216, 64, 128 ?
                print(f"self.inter_B, in_ch ={encoder_out_ch}, ae_ch = {ae_dims}, ae_out_ch = {ae_dims*2}")                

                inter_out_ch = self.inter_AB.get_out_ch(); print(f"inter_out_ch = {inter_out_ch}") #inter_out_ch = ? get_out_ch 

                inters_out_ch = inter_out_ch*2; print(f"inters_out_ch = {inters_out_ch}")                
                self.decoder = model_archi.Decoder(in_ch=inters_out_ch, d_ch=d_dims, d_mask_ch=d_mask_dims, name='decoder')
                print(f"self.decoder = in_ch={inters_out_ch}, d_ch={d_dims}, d_mask_ch={d_mask_dims}")   
                self.model_filename_list += [ [self.encoder,  'encoder.npy'],
                ...
                self.encoder.summary() #26-4-2022
                print("INTER_AB::")
                self.inter_AB.summary()   
                print("INTER_B::")
                self.inter_B.summary()   
                print("DECODER::")
                self.decoder.summary() 
            
            
            #5-5-2022 Should include more parameters, possibly in two lines etc. Should count the number of items/images processed, batch_size*each iteration and show it, not displaying just the iteration (possibly varying batch sizes)
            #os.environ["dfl_model_parameters_string"] = "SAEHDBW-"+self.options['archi']+"-ft:"+self.options['face_type']+"-R-"+ str(resolution)+"-AE-EDM"+f"{ae_dims}|{e_dims}|{d_dims}|{d_mask_dims}"+"-PRE-"+str(self.pretrain)
            #os.environ["dfl_model_parameters_string"] = " " + self.options['archi']+self.options['face_type']+"-R"+ str(resolution)+"AEDM"+f"{ae_dims}|{e_dims}|{d_dims}|{d_mask_dims}"+".PR-"+str(self.pretrain)
            #os.environ["dfl_model_parameters_string"] = " " + "B:"+str(self.batch_size)+" "+self.options['archi']+" "+self.options['face_type']+" R:"+ str(resolution)+" AEDM["+f"{ae_dims}|{e_dims}|{d_dims}|{d_mask_dims}]"+" Prе:"+str(self.pretrain)[0:2]
            i_pretrain=1 if self.pretrain else 0
            os.environ["dfl_model_parameters_string"] = " " + "B:"+str(self.batch_size)+" "+self.options['archi']+" "+self.options['face_type']+" R:"+ str(resolution)+" AEDM["+f"{ae_dims}|{e_dims}|{d_dims}|{d_mask_dims}]"+" Prе:"+str(i_pretrain);
        ...
        if self.is_training:
                ...
                with tf.device( f'/{devices[gpu_id].tf_dev_type}:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    with tf.device(f'/CPU:0'):
                        ...
                        if use_color_input_and_grayscale_model:
                            ...
                            gpu_target_dstm_em  = self.target_dstm_em[batch_slice,:]gpu_warped_src      = self.warped_src [batch_slice,:]
                            """
                            gpu_warped_src      = self.warped_src [batch_slice]
                            gpu_warped_dst      = self.warped_dst [batch_slice]
                            gpu_target_src      = self.target_src [batch_slice]
            ...
            def AE_view(warped_src, warped_dst):
                #print(f"AE_view: np.sum(warped_src), np.sum(warped_dst) = {np.sum(warped_src)}, {np.sum(warped_dst)}")
                if debug_AE_view: print(f"AE_view: Model.py SAEHDBW: warped_src.shape, warped_dst.shape={warped_src},{warped_dst}") 
            ...
            with tf.device( nn.tf_default_device_name if len(devices) != 0 else f'/CPU:0'):
                if 'df' in archi_type:
                    ...
                    if debug_AE_view:  print("AFTER Initializing merge function, elif liae in archi_type::")
            ...
            def AE_merge( warped_dst):
                if debug_AE_view: print(f"AE_merge: warped_dst={warped_dst.shape}\ngpu_pred_src_dst={gpu_pred_src_dst.shape}, gpu_pred_dst_dstm={gpu_pred_dst_dstm.shape},gpu_pred_src_dstm={gpu_pred_src_dstm.shape}")
                if debug_AE_view: print("Before: return nn.tf_sess.run") #**** shape
        ...
        ###############
        if debug_AE_view: print("BEFORE if self.is_training:")
...
#25-4-2022: Back to BGR -> It doesn't make any difference?
    ...
    def export_dfm (self): #not called? --> export for DeepFaceLive! Check! #25-4-2022
        print("export_dfm")
        ...
        with tf.device (nn.tf_default_device_name):
            ...
            warped_dst = tf.placeholder (nn.floatx, (None, self.resolution, self.resolution, export_channels), name='in_face') #25-4-2022            
    ...
    def onTrainOneIter(self):
        ...
        ( (warped_src, target_src, target_srcm, target_srcm_em), \
          (warped_dst, target_dst, target_dstm, target_dstm_em) ) = self.generate_next_samples() #25-4-2022: Here it takes samples, check again! See what's the content of the masks.
        
        if bDebug_onTrainOneIter:
          print("{warped_src.shape}, {target_src.shape}, {target_srcm.shape}, {target_srcm_em.shape}, {warped_dst.shape}, {target_dst.shape}, {target_dstm.shape}, {target_dstm_em.shape}")
          print(f"({warped_src.shape}, {target_src.shape}, {target_srcm.shape}, {target_srcm_em.shape}, {warped_dst.shape}, {target_dst.shape}, {target_dstm.shape}, {target_dstm_em.shape}")
          print(f"({np.sum(warped_src)}, {np.sum(target_src)}, {np.sum(target_srcm)}, {np.sum(target_srcm_em)}, {np.sum(warped_dst)}, {np.sum(target_dst)}, {np.sum(target_dstm)}, {np.sum(target_dstm_em)}")
          #target_srcm=warped_src / 255.0; #Let's see...
          #target_dstm=warped_dst / 255.0; 
          #It gets masks and they are not empty #25-4-2022
                
        src_loss, dst_loss = self.src_dst_train (warped_src, target_src, target_srcm, target_srcm_em, warped_dst, target_dst, target_dstm, target_dstm_em)
        if bDebug_onTrainOneIter: print(f"src_loss, dst_loss = {src_loss}, {dst_loss}")
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        if bDebugModelSAEHDBW_onGetPreview:
          print("onGetPreview.Model.py.SAEHDBW")
          print("SUM: ((warped_src, target_src, target_srcm, target_srcm_em),(warped_dst, target_dst, target_dstm, target_dstm_em) ) = samples")
          for s in samples:
            print(f"np.sum({s}) = {np.sum(s)}")
        
        #S, D, SS, DD, DDM, SD, SDM = [ np.clip( nn.to_data_format(x,"NHWC", self.model_data_format), 0.0, 1.0) for x in ([target_src,target_dst] + self.AE_view (target_src, target_dst) ) ]
        S, D, SS, DD, DDM, SD, SDM = [ np.clip( nn.to_data_format(x,"NHWC", self.model_data_format), 0.0, 255.0) for x in ([target_src,target_dst] + self.AE_view (target_src, target_dst) ) ] #if they are > 1? no... #27-4-2022
        
        if bDebugModelSAEHDBW_onGetPreview: print(f"*SUM MASKS: np.sum(DDM) = {np.sum(DDM)}, np.sum(SDM) = {np.sum(SDM)}")
        """
        cv2.imshow("S", S)
        cv2.imshow("D", D)
        cv2.imshow("SS", SS)
        ...
        if bDebugModelSAEHDBW_onGetPreview: print(f"{S.shape}, {D.shape},{SS.shape}, {DD.shape}, {DDM.shape}, {SD.shape}, {SDM.shape}")
        ...
        #nn.to_data_format !!
        if bDebugModelSAEHDBW_onGetPreview: print(f"target_srcm, target_dstm\n{target_srcm}\n{target_dstm}")
        
        if bDebugModelSAEHDBW_onGetPreview:
          print(f"*SUM MASKS AFTER to_data_format(x, NHWC)...: np.sum(target_srcm), sum(target_dstm)={np.sum(target_srcm)}, {np.sum(target_dstm)}")
        ...
        if self.resolution <= 256:
            ...
            if bDebugModelSAEHDBW_onGetPreview: print(f"n_samples={n_samples}")
            st = []
            for i in range(n_samples):
                ar = S[i], SS[i], D[i], DD[i], SD[i]
                # print(f"ar.shape = {ar.shape}")   tuple
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW', np.concatenate (st, axis=0 )), ]
            if bDebugModelSAEHDBW_onGetPreview:  print(f"{result} ... concatenated {n_samples}")

            st_m = []
            if bDebugModelSAEHDBW_onGetPreview: print(f"n_samples={n_samples}")
            for i in range(n_samples): #MASKED PREVIE - something wrong in BW axis
                if bDebugModelSAEHDBW_onGetPreview: print(f"i={i}")
                SD_mask = DDM[i]*SDM[i] if self.face_type < FaceType.HEAD else SDM[i]                
                if bDebugModelSAEHDBW_onGetPreview: print(f"SD_mask = {SD_mask}")                
                if bDebugModelSAEHDBW_onGetPreview: print(f"SD_mask.shape: {SD_mask.shape}")
                ar = S[i]*target_srcm[i], SS[i], D[i]*target_dstm[i], DD[i]*DDM[i], SD[i]*SD_mask
                #print(f"ar = {ar.shape}") it's a tuple
                if bDebugModelSAEHDBW_onGetPreview: print(f"len(ar) = {len(ar)}") #it's a tuple
                if bDebugModelSAEHDBW_onGetPreview: print(f"len(st_m) = {len(st_m)}") 
                
                st_m.append ( np.concatenate ( ar, axis=1) ) #original
                #st_m.append ( np.concatenate ( ar[0:2], axis=1) )   #was - but output is transformed, #27-4-2022 note
                #print(f"ar[0:2].shape={ar[0:2].shape}") tuple
                if bDebugModelSAEHDBW_onGetPreview: print(f"==>st_m.append = {len(st_m)}") 
                #st_m is a list  print(f"Model.py:get_previews:for i in range(n_samples):... result.shape={st_m.shape}")
                """
            ...
            result += [ ('SAEHDBW masked', np.concatenate (st_m, axis=0 )), ]
        else:
            result = []
            ...
            for i in range(n_samples):
                ar = S[i], SS[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW src-src', np.concatenate (st, axis=0 )), ]

            st = []
            for i in range(n_samples):
                ar = D[i], DD[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW dst-dst', np.concatenate (st, axis=0 )), ]

            st = []
            for i in range(n_samples):
                ar = D[i], SD[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW pred', np.concatenate (st, axis=0 )), ]
            ...
            for i in range(n_samples):
                ...
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW masked src-src', np.concatenate (st_m, axis=0 )), ]

            st_m = []
            for i in range(n_samples):
                ar = D[i]*target_dstm[i], DD[i]*DDM[i]
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW masked dst-dst', np.concatenate (st_m, axis=0 )), ]
            ...
            for i in range(n_samples):
                ...
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('SAEHDBW masked pred', np.concatenate (st_m, axis=0 )), ]
            
            if bDebugModelSAEHDBW_onGetPreview: print(f"Model.py:get_previews:result.shape=0,1=0={result[0].shape},1= {result[1].shape}")
            
           
        #7-5-2022
        """
        filepath="G:\\lastpreview.jpg"
        jpeg_quality_preview = 90
        cv2_imwrite(filepath, result[0], [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_quality_preview ] )     
        """
        return result

    def predictor_func (self, face=None): #Edit, check, adjust for 1-channel, #18-5-2022 --> AE_merge(...)
        print(f"Model.py_SAEHDBW:predictor_func()\nself.model_data_format={self.model_data_format}")
        if use_bw_input:  #actually bw_model?
            face = nn.to_data_format(face[None,...], self.model_data_format, "NCHW") #"NCHW") #18-5-2022 NHWC--> axes don't match array
            #face = nn.to_data_format(face[None,...],   self.model_data_format, "NHWC") #"NCHW")
            #None = np.newaxis
        else:
          face = nn.to_data_format(face[None,...], self.model_data_format, "NHWC")
          
          
        if use_bw_input: #"NCHW" Always should be? the model outputs BW #18-5-2022
            print("if use_bw_input: before  face = face[np.newaxis,...] #?")
            print(f"{face.shape} ==>")
            face = face[np.newaxis,...] #?
            #face = face[...,np.newaxis] #1, 192, 192#? for warpAffine
            #face = face[1:] #? NO
            #face = np.squeeze(face)
            
            #ValueError: Cannot feed value of shape (1, 192, 192, 1) for Tensor 'warped_dst:0', which has shape '(?, 1, 192, 192)'
            print(f"{face.shape}")
            bgr, mask_dst_dstm, mask_src_dstm = [ nn.to_data_format(x,"NCHW", self.model_data_format).astype(np.float32) for x in self.AE_merge(face) ]
            print(f"bgr.shape={bgr.shape}")
            #bgr = np.squeeze(bgr)
            #bgr = np.squeeze(bgr)
            print(f"bgr.shape={bgr.shape}, mask_dst_dstm.shape={mask_dst_dstm.shape}, mask_src_dstm={mask_src_dstm}, bgr.sum={np.sum(bgr)}")
            #cv2.imshow("predictor", bgr*255); cv2.waitKey(0)
            cv2.waitKey(0)
        else:
