...
#22-4-2022 --> set it from main init etc... 
import os
#os.environ["color_input_and_grayscale_model"] = "1" 
use_color_input_and_grayscale_model = "color_input_and_grayscale_model" in os.environ;
bDebugSample_load_bgr = False
print_load_bgr = "print_load_bgr" in os.environ #3-5-2022
...
class Sample(object):
    ...
    def load_bgr(self):
        #22-4-2022
        #use_color_input_and_grayscale_model = "color_input_and_grayscale_model" in os.environ;
        #must be checked once per run? in constructor?        
        if print_load_bgr: print(f"Sample.py:load_bgr: {self.filename}")
        if use_color_input_and_grayscale_model:
           img = cv2_imread_color_as_grayscale (self.filename, loader_func=self.read_raw_file).astype(np.float32) / 255.0  
           if bDebugSample_load_bgr: print("load_bgr grayscale...")
           #cv2.imshow("load_bgr",img) #23-4-2022
           #cv2.waitKey(1)
        else: #if it is single channel, it is read automatically correctly          
