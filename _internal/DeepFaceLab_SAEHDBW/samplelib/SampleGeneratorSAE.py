...
class SampleGeneratorSAE(SampleGeneratorBase):
    ...
    def batch_func(self, param ):
        ...
        def gen_sample(sample, target_face_type, resolution, allow_flip=False, scale=1.0, ct_mode=None, ct_sample=None):#:, tx, ty, rotation, scale):
            print("SampleGeneratorSAE.py::gen_sample")
            ...
            face = sample.load_bgr() #CHECK CHANNELS
            h,w,c = face.shape
            print(f"h,w,c = {h},{w},{c}")
            ...
            warped_face = face
            print(f"SUM(warped_face==face)=np.sum(warped_face)")
            
            if ct_mode is not None: #for now use with none for BW!
            ...
            print(f"SUM(warped_face... warpAffin)={np.sum(warped_face}")
            ...
            print(f"SUM(warped_face... np.clip)={np.sum(warped_face}")
            ...
            print(f"SUM(warped_face... np.warpAffine again)={np.sum(warped_face}")
            ...
            if face_mask is not None:
                ...
                print(f"face_mask=LandmarksProcessor.get_image_hull_mask, face.shape={face.shape}, face_lmrks={face_lmrks}, eybrows_expand_mod={eybrows_expand_mod}")
                print(f"np.sum(face_mask)={np.sum(face_mask)}")
                
            face_mask = np.clip(face_mask, 0, 1)
            print(f"np.clip(face_mask, 0,1) = np.sum(face_mask)={np.sum(face_mask)}")
            ...
            print(f"target_face_mask = cv2.warpAffine...  = {np.sum(target_face_mask)}")
            
            target_face_mask = imagelib.normalize_channels(target_face_mask, 1)
            
            print(f"target_face_mask = imagelib.normalize_channels = {np.sum(target_face_mask)}\ntarget_face_mask.shape= = {target_face_mask.shape=}")
