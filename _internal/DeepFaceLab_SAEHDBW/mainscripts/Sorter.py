...
use_bw_input = "use_bw_input" in os.environ #30-4-2022, Twenkid
...
class HistSsimSubprocessor(Subprocessor):
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            for x in data:
                img = cv2_imread(x)                
                if use_bw_input: img_list.append ([x, cv2.calcHist([img], [0], None, [256], [0, 256])]) #30-4-2022
                else:
...
def sort_by_hist_dissim(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        if use_bw_input: #30-4-2022
           print (f"{str(filepath)}, imae.shape={image.shape}")
           img_list.append ([str(filepath), cv2.calcHist([image], [0], None, [256], [0, 256]), 0 ])
        else:
...
def sort_by_brightness(input_path):
    ...
    if not use_bw_input:
       ...
       img_list = [ [x, np.mean(cv2_imread(x))] for x in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading") ]
    io.log_info ("Sorting...")
    img_list = sorted(img_list, key=operator.itemgetter(1), reverse=True)
    return img_list, []

def sort_by_hue(input_path): #don't use for BW #29-4-2022
