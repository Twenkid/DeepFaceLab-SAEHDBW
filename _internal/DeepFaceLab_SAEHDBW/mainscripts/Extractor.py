...
extract_to_bw = "extract_to_bw" in os.environ #29-4-2022 --> for extraction; normalize channels --> to 1
print(f"Extractor.extract_to_bw={extract_to_bw}")
#extract_to_bw = True #forced
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if cached_filepath != filepath:
                image = cv2_imread( filepath ) #if bw_ it will read bw
                if image is None:
                    self.log_err (f'Failed to open {filepath}, reason: cv2_imread() fail.')
                    return data
                #if extract_to_bw: image = imagelib.normalize_channels(image, 1) #grayscale - no, the recognizer wants color: (?,?,?,3) --> then normalize_channels after recognition? will it work? #29-4-2022
                #else: image = imagelib.normalize_channels(image, 3) #color
                
                if extract_to_bw: image = imagelib.normalize_channels(image, 1) #grayscale
                else: image = imagelib.normalize_channels(image, 3) #color
                image = imagelib.cut_odd_image(image)
                self.cached_image = ( filepath, image )

            h, w, c = image.shape #debug, check
        ...
        def final_stage(data,
            ...
            for rect, image_landmarks in zip( rects, landmarks ):
                ...
                if extract_to_bw: 
                    #print("\nfinal_stage: extract_to_bw:")
                    #imagelib.normalize_channels(face_image, 1)
                    #print(imagelib.shape)
                    #It should be grayscale already, so all channels are equal
                    cv2_imwrite(output_filepath, face_image[:,:,0], [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_quality ] ) #OK! normalize channels should do the same, but is not needed
                    #29-4-2022                    
                else:
                     cv2_imwrite(output_filepath, face_image, [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_quality ] )
                #print(f"cv2_imwrite:{output_filepath}")
...
def main(detector=None,
    ...
    print("mainscripts\Extractor.py\main...")    
