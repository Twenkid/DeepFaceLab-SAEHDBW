...
resize_color_to_grayscale = "resize_color_to_grayscale" in os.environ #12.5.2022
print(f"use_bw_input={use_bw_input}\nresize_color_to_grayscale={resize_color_to_grayscale}")
only_convert_to_grayscale = "only_convert_to_grayscale" in os.environ
            ...
            try:
                ...
                if dflimg is None or not dflimg.has_data():
                    ...
                    if face_type is not None:
                        ...
                        if resize_color_to_grayscale: #12-5-2022
                        ...
                        if resize_color_to_grayscale and not use_bw_input: #12-5-2022
                           #if img.shape == 3: #do not convert if there are grayscale images by mistake; test it
                             img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                          
                        if not only_convert_to_grayscale:
...
def process_folder ( dirpath):
    ...
    if only_convert_to_grayscale: print("\nWarning: only_convert_to_grayscale is True, the new image size and face_type will be ignored and the images will be just converted to grayscale!")
    image_size = io.input_int(f"New image size", 512, valid_range=[128,2048]) #12-5-2022
    
    face_type = io.input_str ("Change face type", 'same', ['h','mf','f','wf','head','same']).lower()
    
    #if only_convert_to_grayscale: face_type = None #12-5-2022
    if only_convert_to_grayscale: face_type = 'same' #12-5-2022
